class audioProcessor extends AudioWorkletProcessor {
	constructor(...args) {
		...
		this.DMode = 'Bytebeat';
		this.externalAudio = null; // <-- Add all properties first
        this.audioLength = 0; // Ensure extensible before seal
		Object.seal(this);         // <-- Then seal the object
	...
	receiveData(data) {
		...
		// Expose audioLength as a global variable if sent from main thread, or set default if not
		if (data.audioLength !== undefined) {
			this.audioLength = data.audioLength;
		} else if (typeof this.audioLength === 'undefined') {
			this.audioLength = 0;
		}
		Object.defineProperty(globalThis, 'audioLength', {
			value: this.audioLength,
			writable: false,
			configurable: true
		});
		...
		// For external audio input
        if (data.externalAudio) {
            this.externalAudio = data.externalAudio;
        }
	...
	setFunction(codeText) {
		...
		// Add 'audioIN' as a parameter instead of 'audio'
		params.push('int', 'window', ...chyxNames, 'audioIN');
		// Provide a safe audioIN(index, channel) function
		values.push(Math.floor, globalThis, ...chyxFuncs, (index, channel) => {
			const data = this.externalAudio?.data;
			if (Array.isArray(data) && Array.isArray(data[index])) {
				const v = data[index][channel];
