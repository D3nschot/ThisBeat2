...
globalThis.bytebeat = new class {
	...
	async init() {
		...
		// !window.location.hostname.includes`d3nschot.github.io`

		if (false) { // You may also change this line to your hostname. (instead of `chasyxx`, use `t-8492` for example)
	...
	async parseUrl() {
		...
		// If hash is missing or does not contain #EnBeat2- or #v3b64, load Default.tb2 and return
		if (!hash || (!hash.includes('#EnBeat2-') && !hash.startsWith('#v3b64'))) {
			await this.tryToLoadDefault();
			return;
		}
		let songData;
	...
	async tryToLoadDefault() {
		this.beginLoad();
		try {
			const response = await fetch('./Default.tb2', { cache: 'no-cache' });
			if (response.status !== 200 && response.status !== 304) {
				console.error(`Couldn't load Default.tb2: ${response.status} ${response.statusText}`);
				this.endLoad();
				return;
			}
			...
			// Load code
			const code = await zip.file('code.txt').async('string');
			// Load settings
			let settings = {};
			if (zip.file('settings.json')) {
				settings = JSON.parse(await zip.file('settings.json').async('string'));
			}
			// Load external audio
			let externalAudioBuffer = null;
			if (zip.file('audio.json')) {
				const audioJson = await zip.file('audio.json').async('string');
				if (audioJson) {
					externalAudioBuffer = JSON.parse(audioJson);
				}
			}
			// Set code and settings
			this.loadCode({ code, sampleRate: settings.sampleRate, mode: settings.mode }, false);
			if (settings.divisor) this.setSampleDivisor(settings.divisor);
			// Set external audio
			if (externalAudioBuffer) {
				this.audioWorkletNode?.port.postMessage({
					externalAudio: externalAudioBuffer,
					audioLength: externalAudioBuffer.data ? externalAudioBuffer.data.length : 0
				});
			}
			this.endLoad();
		} catch (err) {
			console.error(`Couldn't load Default.tb2: ${err}`);
			this.endLoad();
		}
	}
...
let externalAudioBuffer = null;
let audioLength = 0;
Object.defineProperty(globalThis.bytebeat, 'audioLength', {
	get() { return audioLength; },
	configurable: true
});
// Expose audioLength as a global variable for formulas
Object.defineProperty(globalThis, 'audioLength', {
	get() { return audioLength; },
	configurable: true
});
...
document.getElementById('audio-file-input').onchange = function(e) {
	...
	reader.onload = function(ev) {
		bytebeat.audioCtx.decodeAudioData(ev.target.result, function(buffer) {
			// Send buffer to AudioWorklet
			// Transpose channel data to [sample][channel]
			const channels = buffer.numberOfChannels;
			const length = buffer.length;
			const channelData = Array.from({length: channels}, (_, ch) => buffer.getChannelData(ch));
			const transposed = Array.from({length}, (_, i) =>
				channelData.map(chArr => chArr[i])
			);
			...
			externalAudioBuffer = {
			   ...
			   audioLength = externalAudioBuffer.data ? externalAudioBuffer.data.length : 0;
			if (bytebeat.audioWorkletNode) {
				bytebeat.audioWorkletNode.port.postMessage({
					externalAudio: externalAudioBuffer,
...
// --- .tb2 Project Export/Import Logic ---
document.getElementById('export-tb2').onclick = async function() {
	const zip = new JSZip();
	// Save code from editor
	zip.file('code.txt', bytebeat.editorValue);
	// Save settings (mode, sampleRate, divisor)
	zip.file('settings.json', JSON.stringify({
		mode: bytebeat.songData.mode,
		sampleRate: bytebeat.songData.sampleRate,
		divisor: bytebeat.controlSampleDivisor?.value || 1
	}));
	// Save external audio buffer if present and valid
	if (externalAudioBuffer && externalAudioBuffer.data && externalAudioBuffer.data.length > 0) {
		zip.file('audio.json', JSON.stringify(externalAudioBuffer));
	}
	const content = await zip.generateAsync({type: 'blob'});
	bytebeat.saveData(content, 'project.tb2');
};

document.getElementById('import-tb2-btn').onclick = function() {
	document.getElementById('import-tb2').click();
};

document.getElementById('import-tb2').onchange = function(e) {
	const file = e.target.files[0];
	if (!file) return;
	// Reset player and clear external audio before loading
	bytebeat.resetTime();
	externalAudioBuffer = null;
	JSZip.loadAsync(file).then(async zip => {
		// Load code
		const code = await zip.file('code.txt').async('string');
		if (bytebeat.editorElem) bytebeat.editorElem.value = code;
		if (bytebeat.editorView) bytebeat.editorView.dispatch({changes: {from: 0, to: bytebeat.editorView.state.doc.length, insert: code}});
		// Load settings
		const settings = JSON.parse(await zip.file('settings.json').async('string'));
		if (settings.mode) bytebeat.setPlaybackMode(settings.mode);
		if (settings.sampleRate) bytebeat.setSampleRate(settings.sampleRate);
		if (settings.divisor) bytebeat.setSampleDivisor(settings.divisor);
		// Load external audio
		if (zip.file('audio.json') && await zip.file('audio.json').async('string')) {
			externalAudioBuffer = JSON.parse(await zip.file('audio.json').async('string'));
				audioLength = externalAudioBuffer.data ? externalAudioBuffer.data.length : 0;
			// Send both externalAudio and audioLength to processor
			if (bytebeat.audioWorkletNode) {
				bytebeat.audioWorkletNode.port.postMessage({
					externalAudio: externalAudioBuffer,
					audioLength: audioLength
		...
		// Ensure player is stopped and function is refreshed
		bytebeat.playbackStop();
